// Package csv reads comma-separated values (CSV) files.  There are many kinds
// of CSV files; this package supports the format described in RFC 4180.
//
...
func (r *Reader) readRune() (rune, error) {
	r1, runeSize, err := r.r.ReadRune()

	// Handle \r\n here. We make the simplifying assumption that
	// anytime \r is followed by \n that it can be folded to \n.
	// We will not detect files which contain both \r\n and bare \n.
	if r1 == '\r' {
		var peekRuneSize int
		r1, peekRuneSize, err = r.r.ReadRune()
		if err == nil {
			if r1 != '\n' {
				r.r.UnreadRune()
				r1 = '\r'
			} else {
				r.ByteOffset += int64(peekRuneSize)
			}
		}
	}
	r.ByteOffset += int64(runeSize)
	r.column++
	return r1, err
}
